...
if (!class_exists('YtMegaNewsIIReader')){
	class YtMegaNewsIIReader{
		...
		public function getList(&$params){
			...
			$secs = array();
			$cats = array();
			if (!isset($params->source) || !is_array($params->source) || count($params->source)==0){
				return $sections;
			}
			...
			foreach ($rows as $k => $row){
				...
				$rows[$k]->description = isset($row->introtext) && !empty($row->introtext)
					? $row->introtext
					: $row->fulltext;
				$rows[$k]->image_urls = YtUtils::extractImages(& $rows[$k]->description);
				
				$rows[$k]->url = isset($customUrl['article'][$k])
					? $customUrl['article'][$k]
					: JRoute::_( ContentHelperRoute::getArticleRoute($row->id, $cid, $sid) );
		...
		private function _getCustomUrl($str){
			if(file_exists(dirname(__FILE__).DS.'customurl.php')){
				include_once dirname(__FILE__).DS.'customurl.php';
				if (function_exists('build_custom_url')){
					return build_custom_url($str);
				}
			} else {
