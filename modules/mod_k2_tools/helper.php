<?php
/**
 * @version		$Id: helper.php 1314 2011-11-02 15:51:14Z lefteris.kavadas $
 * @package		K2
...
class modK2ToolsHelper {
	...
	function getAuthors(&$params) {
		...
		if(K2_JVERSION=='16'){
			...
			{$languageCheck}
	        AND EXISTS (SELECT * FROM #__k2_categories WHERE id= #__k2_items.catid AND published=1 AND trash=0 AND access IN(".implode(',', $user->authorisedLevels()).") {$languageCheck})";        	
		}
		else {
			$query = "SELECT DISTINCT created_by FROM #__k2_items
	        WHERE {$where} published=1 
		...
		if (count($rows)) {
			foreach ($rows as $row) {
				...
				if(K2_JVERSION=='16'){
			        ...
			        AND i.published = 1
			        AND i.access IN(".implode(',', $user->authorisedLevels()).") 
			        AND ( i.publish_up = ".$db->Quote($nullDate)." OR i.publish_up <= ".$db->Quote($now)." )
			        AND ( i.publish_down = ".$db->Quote($nullDate)." OR i.publish_down >= ".$db->Quote($now)." )
			        AND i.trash = 0 AND created_by_alias='' AND c.published = 1 AND c.access IN(".implode(',', $user->authorisedLevels()).") AND c.trash = 0 {$languageCheck} ORDER BY created DESC";                 	
				}
				else {
					$query = "SELECT i.*, c.alias as categoryalias FROM #__k2_items as i
			        LEFT JOIN #__k2_categories c ON c.id = i.catid
			...
			switch ($view) {
				...
				case 'item':
					if(K2_JVERSION=='16'){
						...
						$query = "SELECT * FROM #__k2_items  WHERE id={$id} AND published=1 AND trash=0 AND access IN(".implode(',', $user->authorisedLevels()).") {$languageCheck} AND EXISTS (SELECT * FROM #__k2_categories WHERE #__k2_categories.id= #__k2_items.catid AND published=1 AND access IN(".implode(',', $user->authorisedLevels()).")  {$languageCheck} )";
					}
					else {
...
class MyCalendar extends Calendar {
	...
	function getDateLink($day, $month, $year) {
		...
		$languageCheck = '';
		if(K2_JVERSION=='16'){
			$accessCheck = " access IN(".implode(',', $user->authorisedLevels()).") ";
			if($mainframe->getLanguageFilter()) {
				$languageTag = JFactory::getLanguage()->getTag();
				$languageCheck = " AND language IN (".$db->Quote($languageTag).", ".$db->Quote('*').") ";
			}
	...
	function getCalendarLink($month, $year) {
		$itemID = JRequest::getInt('Itemid');
		if ($this->category > 0)
		return JURI::base(true)."/index.php?option=com_k2&amp;view=itemlist&amp;task=calendar&amp;month={$month}&amp;year={$year}&amp;catid={$this->category}&amp;Itemid={$itemID}";
		else
		return JURI::base(true)."/index.php?option=com_k2&amp;view=itemlist&amp;task=calendar&amp;month=$month&amp;year=$year&amp;Itemid={$itemID}";
	}
}
